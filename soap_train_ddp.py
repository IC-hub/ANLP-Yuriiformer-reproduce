"""DDP training script for SOAP+Lie-Trotter SOAPFormer on TinyStories (multi-GPU).
...
def main():
    ...
    if rank == 0:
        config = {
            "model": "SOAPFormer-small",
            "variant": "SOAP+Lie-Trotter",
        ...
        if rank == 0:
            ...
            if step % VAL_INTERVAL == 0 or step == TOTAL_STEPS - 1:
                ...
                for i, layer in enumerate(raw_model.layers):
                    ...
                    log_dict[f"scalars/layer{i}_betaR_attn"] = torch.sigmoid(layer.betaR_attn_raw).item()
                    ...
                    log_dict[f"scalars/layer{i}_betaR_mlp"] = torch.sigmoid(layer.betaR_mlp_raw).item()
