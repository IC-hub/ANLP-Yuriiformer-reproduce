"""DDP training script for AdamW+Lie-Trotter AdamWFormer on OpenWebText (multi-GPU).

Supports checkpoint resume via --resume flag.
Uses torch.compile for faster training.
Checkpoints saved to $CACHE (compute-node local storage).
"""
...
# ── Hyperparameters (OWT, from paper Table 1) ───────────────────────────────
TOTAL_STEPS = 30_000
WARMUP_STEPS = 3_000
BATCH_SIZE = 8
...
# Optimizer LRs (OWT uses lower Muon LR)
MUON_LR = 0.004
ADAMW_LR = 6e-4
SCALAR_LR = 3e-3
...
CKPT_DIR = os.path.join(CACHE, "checkpoints_adamw_owt")
...
def main():
    ...
    if rank == 0:
        config = {
            "model": "AdamWFormer-small-OWT",
            "dataset": "OpenWebText",
            "variant": "AdamW+Lie-Trotter",
        ...
        wandb.init(project="adamwformer-owt", config=config, resume="allow")
        ...
        if rank == 0:
            ...
            if step % VAL_INTERVAL == 0 or step == TOTAL_STEPS - 1:
                ...
                for i, layer in enumerate(raw_model.layers):
                    ...
                    log_dict[f"scalars/layer{i}_lambda_attn"] = torch.sigmoid(layer.lambda_attn_raw).item()
                    ...
                    log_dict[f"scalars/layer{i}_lambda_mlp"] = torch.sigmoid(layer.lambda_mlp_raw).item()
