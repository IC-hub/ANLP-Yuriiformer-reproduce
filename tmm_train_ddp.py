"""DDP training script for TMM+Lie-Trotter TMMFormer on TinyStories (multi-GPU)."""
...
def main():
    ...
    if rank == 0:
        config = {
            "model": "TMMFormer-small",
            "variant": "TripleMomentum+Lie-Trotter",
        ...
        wandb.init(project="tmmformer", config=config, resume="allow")
        ...
        if rank == 0:
            ...
            if step % VAL_INTERVAL == 0 or step == TOTAL_STEPS - 1:
                ...
                for i, layer in enumerate(raw_model.layers):
                    log_dict[f"scalars/L{i}_mu_attn"] = torch.sigmoid(layer.mu_attn_raw).item()
                    log_dict[f"scalars/L{i}_beta_attn"] = torch.sigmoid(layer.beta_attn_raw).item()
                    log_dict[f"scalars/L{i}_gamma_attn"] = F.softplus(layer.gamma_attn_raw).item()
                    log_dict[f"scalars/L{i}_nu_attn"] = F.softplus(layer.nu_attn_raw).item()
                    log_dict[f"scalars/L{i}_mu_mlp"] = torch.sigmoid(layer.mu_mlp_raw).item()
                    log_dict[f"scalars/L{i}_beta_mlp"] = torch.sigmoid(layer.beta_mlp_raw).item()
                    log_dict[f"scalars/L{i}_gamma_mlp"] = F.softplus(layer.gamma_mlp_raw).item()
                    log_dict[f"scalars/L{i}_nu_mlp"] = F.softplus(layer.nu_mlp_raw).item()
